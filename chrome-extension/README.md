# Quick-calendar Chrome 拡張

ブラウザで**テキストを選択 → 右クリック → 「予定を追加」** で、
Quick-calendar — 同じ精度・同じカレンダーに予定を登録できる Chrome 拡張（Manifest V3）。
...
| **GAS WebアプリURL** | Quick-calendar の GAS をデプロイした「ウェブアプリURL」（`https://script.google.com/macros/s/...`） |
